...
if(isset($_GET['id']) && $_GET['id'] !== ''){
  ...
  if ($db->query($update_hits)) {
    ...
    $typ = 'moto';
      ...
      $insert = $db->query($sql);
      header('Location:specificpost.php?id=' .$postid);
...
<body>
  ...
  <?php
            $db = new mysqli("localhost", "root", "","motodb") or die ("Błąd");
            $sql = "SELECT * from images JOIN post_moto ON images.uploaded_on = post_moto.uploaded_on_post WHERE post_id='$postid'"   ;
...
<div class="row">
  ...
  <div class="col-lg-12">
    ...
    <div class="card my-4">
      ...
      <div class="card-body">
      <form action="specificpost.php" method="POST">
    ...
    <?php
      $db = new mysqli("localhost", "root", "","motodb") or die ("Błąd");
      $sql = "SELECT coments.comment, coments.id, coments.comment_sender_name,  coments.coment_post_id, post_moto.post_id, user.number_of_comments FROM `coments` INNER JOIN post_moto ON coments.coment_post_id = post_moto.post_id INNER JOIN user ON coments.comment_sender_name = user.username WHERE post_id='$postid' AND typ = 'moto' ORDER BY coments.id DESC";
      $result= $db->query($sql);
