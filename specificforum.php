...
if(isset($_GET['id']) && $_GET['id'] !== ''){
  ...
  if ($db->query($update_hits)) {
    ...
    $typ = 'forum';
      ...
      $insert = $db->query($sql);
      header('Location:specificforum.php?id=' .$postid);
...
<body>
  ...
  <?php
            $db = new mysqli("localhost", "root", "","motodb") or die ("Błąd");
            $sql = "SELECT * from forum  WHERE forum_id='$postid'"   ;
...
<div class="row">
  ...
  <div class="col-lg-12">
    ...
    <div class="card my-4">
      ...
      <div class="card-body">
      <form action="specificforum.php" method="POST">
    ...
    <?php
      $db = new mysqli("localhost", "root", "","motodb") or die ("Błąd");
      $sql = "SELECT  coments.comment, coments.id, coments.comment_sender_name, coments.coment_post_id, forum.forum_id, user.number_of_comments  FROM coments INNER JOIN forum ON coments.coment_post_id = forum.forum_id INNER JOIN user ON coments.comment_sender_name = user.username WHERE forum_id='$postid' AND typ = 'forum' ORDER BY coments.id DESC";
      $result= $db->query($sql);
